...
if uploaded_file is not None:
    ...
    st.write("**ℹ️ Completed Summary Of Dataset**")
    buffer = io.StringIO()
    df.info(buf=buffer)
    info = buffer.getvalue()
    st.text(info)

    st.write("**📈 Statiscal Summary Of Dataset**")
    st.dataframe(df.describe())

    st.write("**📈 Statiscal Summary Of Non Numerical Features**")
    st.dataframe(df.describe(include='object'))

    st.write("**🕵🏻‍♂️ Select Your Desired Columbs**")
    ...
    with col1:
        lin_btn = st.button("Click To Generate A Line Graph")
    with col2:
        bar_btn = st.button("Click To Generate A Bar Graph")
