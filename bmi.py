import streamlit as st
import pandas as pd
import altair as alt

st.set_page_config(page_title="BMI Calculator",layout="centered")

st.title("💪 BMI Calculator")
st.write("Let's Claculate Your **Body Mass Index** And Understand What It Means")

st.header(" 🖋 Enter Your Details")
height = st.number_input("Please Enter Your Height (In cm)",min_value=50,max_value=280,value=150)
weight = st.number_input("Please Enter Your Weight (In kg)",min_value=10,max_value=300,value=50)

st.write("📏 Your Height :",height,"cm")
st.write("⚖️ Your Height :",weight,"kg")

if st.button("Calculate BMI"):
    h_m = height / 100  #convert cm to meter
    bmi = weight / (h_m ** 2)
    st.success(f" 📍 Your BMI Is : **{bmi:.2f}**") 

    # BMI Category
    if bmi <18.5:
        category = "Underweight 🫣"
        color = "#1359F1"
    
    elif 18.5 <= bmi < 25:
        category = "Normal 😀"
        color = "#0EEC21"

    elif 25<= bmi < 30:
        category = "Overweight 😯"
        color = "#E8EC0E"

    else:
        category = "Obese 😬"
        color = "#F00909"

    st.markdown(
        f"""
        <div style='background-color:{color};text-align:'center>
        <h3>Your BMI Category {category}</h3>
        </div>""",unsafe_allow_html=True
    )

st.header("📊 BMI Range Chart")
bmi_data = pd.DataFrame({"Category":["Underweight","Normal","Overweight","Obese"],
                         "Range":[18.5,24.9,29.9,40]})

# Define custom colors for each category
color_scale = alt.Scale(
    domain=["Underweight", "Normal", "Overweight", "Obese"],
    range=["#1359F1", "#0EEC21", "#E8EC0E", "#F00909"]
)
 
# Create chart
chart = (
    alt.Chart(bmi_data)
    .mark_bar()
    .encode(
        x=alt.X("Category:N", title="BMI Category"),
        y=alt.Y("Range:Q", title="BMI Range"),
        color=alt.Color("Category:N", scale=color_scale, legend=None)
    )
    .properties(width=600, height=400)
)
 
st.altair_chart(chart, width='stretch')